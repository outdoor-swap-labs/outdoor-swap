...
itemController.getByCategory = (req, res, next) => {
    ...
    const categoryQuery = 'SELECT * FROM "public"."item" WHERE category = $1';
    const categoryItemsVar = [category]
    db.query(categoryQuery, categoryItemsVar)
      .then(data => {
        console.log(data)
        if (data.rows.length === 0) {
          throw new Error('No item found in that category');
        }
...
itemController.getByLocation = (req, res, next) => {
    ...
    const locationQuery = 'SELECT * FROM "public"."item" WHERE location = $1';
    const locationItemsVar = [location]
    db.query(locationQuery, locationItemsVar)
      .then(data => {
        console.log(data)
        if (data.rows.length === 0) {
          throw new Error('No item found in that location');
        }
        res.locals.items = data.rows;
        return next();
      })
